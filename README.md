...
- 工作台一级导航为「首页、待办事项、到期与提醒、Kindle、出海、AI、X Assistant、设置」。出海集中维护 VPN 快捷链接、手机卡套餐和银行金融服务；AI 下提供完整提示词和短提示词维护，以及本机 Skills 目录浏览；提示词支持分类筛选、预览和复制，短提示词支持快速复制，Skills 支持按 agent 类型筛选、查看 SKILL.md 详情和保存备注。
...
- 设置页展示 SQLite 数据文件路径与大小、SQLite 凭证配置状态、下一次 X 生成时间与只读保留策略，支持 Finder 定位与复制路径；
- 数据保留策略：X 内容与回复可编辑 180 天、只读 30 天后清理；定位与素材保存至主动删除；待办与到期项保存至主动处理、停用、归档或删除；
- 支持手动创建密码加密的完整数据备份与受控恢复，备份排除 SQLite 凭证表。

### 编程记录

- 编程记录支持确认式公开采集、GitHub 仓库来源快照、GitHub Stars 预览导入、重叠统计和人工分类；列表可由用户手动标记资料是否已消化，只有用户确认后才写入本地记录。
- GitHub Personal Access Token、模型 API Key、Finnhub API Key 与 Bark device key 保存于本机 SQLite 凭证表；Stars 导入不会后台同步、定时抓取或把凭证写入加密备份和接口响应。

### 股票

- 股票模块提供市场行情、待开仓股票、仓位管理和重要事件页面；重要事件按美东市场周展示本周及未来三周的 FOMC、通胀、就业、增长、季度期权到期和美国大选等市场级事件，并转换为北京时间显示。
- 重要事件结合 BLS、BEA 官方发布日历与 Federal Reserve FOMC 会议表；Finnhub 仅用于股票报价与估值。官方数据源按来源独立降级，暂时不可用时仍展示本地核心事件。影响说明用于风险提示，不代表涨跌预测或投资建议。
...
- X 下提供「每日推文」：从已维护的完整提示词调用模型，按每种语言的条数和中文/English/中英组合生成可编辑草稿，并支持快速复制。
...
packages/
  ...
  module-ai/            AI 模块（提示词维护、Skills 目录浏览、详情和备注）
...
```text
~/Library/Application Support/workbench/workbench.sqlite
~/Library/Application Support/workbench/stock.sqlite
```

其中 `workbench.sqlite` 保存工作台通用数据，`stock.sqlite` 保存待开仓股票、仓位管理、股票设置、股票代码与行情预警数据；旧的 `X Assistant/x-assistant.sqlite` 会在首次启动时迁移。
...
- API Key 由用户在本地配置，模型 Key 保存于本机 SQLite 凭证表，不应提交到仓库；
