...
import { api, SectionCard, Empty, LoadingButton, statusTagVariant, statusTagLabel } from '@x-assistant/core';

function ExpiringForm({ value, onChange }) {
  return (
...
function ExpiringItemsPage() {
  ...
  const [input, setInput] = useState({ name: '', notes: '', mode: 'once', dueAt: null, intervalValue: 1, intervalUnit: 'day', advanceValue: 0, advanceUnit: 'day' });
  ...
  const save = async () => { setSaving(true); try { const body = { name: input.name, notes: input.notes, mode: input.mode, dueAt: input.dueAt ? input.dueAt.toISOString() : undefined, advanceValue: input.advanceValue, advanceUnit: input.advanceUnit }; if (input.mode !== 'once') { body.intervalValue = input.intervalValue; body.intervalUnit = input.intervalUnit; } await api('/expiring-items', { method: 'POST', body: JSON.stringify(body) }); setInput({ name: '', notes: '', mode: 'once', dueAt: null, intervalValue: 1, intervalUnit: 'day', advanceValue: 0, advanceUnit: 'day' }); setCreateOpen(false); await load(); } finally { setSaving(false); } };
  ...
  const [editingInput, setEditingInput] = useState({ name: '', mode: 'once', dueAt: null, intervalValue: 1, intervalUnit: 'day', advanceValue: 0, advanceUnit: 'day', notes: '' });
  const [editing, setEditing] = useState(false);
  const openEdit = (item) => { setEditingInput({ name: item.name, mode: item.mode, dueAt: item.dueAt ? new Date(item.dueAt) : null, intervalValue: item.intervalValue ?? 1, intervalUnit: item.intervalUnit ?? 'day', advanceValue: item.advanceValue, advanceUnit: item.advanceUnit, notes: item.notes || '' }); setEditingItem(item); };
  const saveEdit = async () => { setEditing(true); try { const body = { name: editingInput.name, mode: editingInput.mode, dueAt: editingInput.dueAt ? editingInput.dueAt.toISOString() : undefined, advanceValue: editingInput.advanceValue, advanceUnit: editingInput.advanceUnit, notes: editingInput.notes }; if (editingInput.mode !== 'once') { body.intervalValue = editingInput.intervalValue; body.intervalUnit = editingInput.intervalUnit; } await api(`/expiring-items/${editingItem.id}`, { method: 'PATCH', body: JSON.stringify(body) }); setEditingItem(null); await load(); } finally { setEditing(false); } };
  ...
  return (
    <SectionCard title="到期与提醒">
      ...
      {loading ? <p className="text-foreground-muted">正在加载到期项…</p> : items.length ? (
        <Table hoverableRows>
          <TableHeader>
            <TableRow><TableHead>项目</TableHead><TableHead>模式</TableHead><TableHead>到期时间</TableHead><TableHead>提前提醒</TableHead><TableHead>状态</TableHead><TableHead>操作</TableHead></TableRow>
          </TableHeader>
          <TableBody>
            {items.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell>{`${item.advanceValue} ${item.advanceUnit === 'minute' ? '分钟' : item.advanceUnit === 'hour' ? '小时' : '天'}`}</TableCell>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:w-110">
          ...
          <DialogBody>
            <ExpiringForm value={input} onChange={setInput} />
          </DialogBody>
          <DialogFooter>
      ...
      <Dialog open={editingItem !== null} onOpenChange={(open) => { if (!open) setEditingItem(null); }}>
        <DialogContent className="sm:w-110">
          ...
          <DialogBody>
            <ExpiringForm value={editingInput} onChange={setEditingInput} />
          </DialogBody>
          <DialogFooter>
            <DialogClose render={<Button variant="soft">取消</Button>} />
