    ...
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-2 gap-3">
        <Field>
          ...
          <Input required name="stock-symbol" autoComplete="off" spellCheck={false} list={symbolListId} value={value.symbol} onChange={updateSymbol} placeholder="输入或选择代码，如 AAPL…" />
...
function StockPositionsPage() {
  ...
  const totalPnlPercent = totalAssets > 0 ? (totalPnl / totalAssets) * 100 : 0;
  ...
  const allocationDeviations = useMemo(() => positions
    ...
    .sort((left, right) => Math.abs(right.difference) - Math.abs(left.difference)), [positions, currentTotalAssets]);
  const trackedAllocationPercent = allocationDeviations.reduce((sum, entry) => sum + entry.currentPercent, 0);
  const largestPosition = useMemo(() => positions.reduce((largest, item) => {
  ...
  const filteredPositions = useMemo(() => positions.filter((item) => {
    ...
    return true;
  }), [positions, valuationFilter, targetFilter, currentTotalAssets]);
  ...
  const openEdit = (item) => { setDetailItem(null); setEditingInput({ symbol: item.symbol, name: item.name, market: item.market, quantity: item.quantity, costPrice: item.costPrice, currentPrice: item.currentPrice, targetPercent: item.targetPercent, notes: item.notes || '' }); setEditingItem(item); };
    ...
    <>
      ...
      <Card>
        ...
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button variant="outline" onClick={() => setOverviewPanel('sold')}>已卖出 {soldPositions.length} 笔</Button>
          </div>
        ...
        <p className="mb-4 text-xs leading-5 text-foreground-muted">配置 Finnhub API Key 后可刷新美股现价；港股与 A 股保留手动维护。完整估值、价格与操作收进持仓详情。</p>
        {priceRefreshMessage && <p className="mb-3 text-sm text-success-emphasis" aria-live="polite">{priceRefreshMessage}</p>}
        {loading ? <p className="text-foreground-muted">正在加载持仓…</p> : positions.length ? sortedPositions.length ? (
                ...
                {sortedPositions.map((item) => {
                  ...
                  return (
                    <TableRow key={item.id}>
                      <TableCell>
                        <div className="flex min-w-0 flex-wrap items-center gap-1.5"><span className="font-medium">{item.symbol}</span><Badge variant="outline" size="sm">{MARKET_LABEL[item.market] || item.market}</Badge></div>
        ...
        ) : <Empty description="没有符合当前筛选条件的持仓。" /> : <Empty description="暂无持仓，点击「新增持仓」添加第一笔" />}
        </div>
      </Card>
      ...
      <aside className="flex min-w-0 flex-col gap-4">
        <SectionCard title="总盈亏">
          <div className="grid grid-cols-2 gap-4">
            <AssetMetric label="持仓浮盈" valueClassName="text-2xl" color={activePnl >= 0 ? 'var(--profit)' : 'var(--loss)'} detail={<AnimatedPercent value={activePnlPercent} />}><AnimatedMoney value={activePnl} /></AssetMetric>
            <AssetMetric label="累计盈亏" valueClassName="text-2xl" color={totalPnl >= 0 ? 'var(--profit)' : 'var(--loss)'} detail={<AnimatedPercent value={totalPnlPercent} />}><AnimatedMoney value={totalPnl} /></AssetMetric>
          </div>
        ...
        <SectionCard title="总资产">
          <div className="grid grid-cols-2 gap-4">
            <AssetMetric label="当前总资产" valueClassName="text-2xl" color={currentTotalAssets >= totalAssets ? 'var(--profit)' : 'var(--loss)'}><AnimatedMoney value={currentTotalAssets} /></AssetMetric>
            <AssetMetric label="现金" valueClassName="text-2xl" color={cash < 0 ? 'var(--loss)' : undefined} detail={<><AnimatedPercent value={cashPercent} /> 占比</>}><AnimatedMoney value={cash} /></AssetMetric>
            <AssetMetric label="持仓" valueClassName="text-2xl" color="var(--profit)" detail={<><AnimatedPercent value={investedPercent} /> 占比</>}><AnimatedMoney value={totalMarketValue} /></AssetMetric>
            <AssetMetric label="初始资金" valueClassName="text-2xl"><AnimatedMoney value={totalAssets} /></AssetMetric>
          </div>
      ...
      <Dialog open={overviewPanel !== null} onOpenChange={(open) => { if (!open) setOverviewPanel(null); }}>
        <DialogContent className={overviewPanel === 'pnl' ? 'sm:w-3/4 sm:max-w-none' : 'sm:max-w-3xl'}>
          <DialogHeader>
            <DialogTitle>{overviewPanel === 'pnl' ? '盈亏构成' : overviewPanel === 'assets' ? '资金分配' : overviewPanel === 'allocation' ? '目标仓位偏差' : '已卖出股票'}</DialogTitle>
            <DialogDescription>{overviewPanel === 'pnl' ? '浮动盈亏、已实现盈亏与单笔贡献。' : overviewPanel === 'assets' ? '初始资金、现金与持仓市值。' : overviewPanel === 'allocation' ? '按实际仓位与目标仓位的偏离程度排序。' : '已完成卖出的历史记录。'}</DialogDescription>
          </DialogHeader>
          <DialogBody>
            {overviewPanel === 'pnl' && <StockStatsPage positions={positions} soldPositions={soldPositions} loading={loading} />}
              ...
              <div className="grid grid-cols-[minmax(0,1fr)_minmax(16rem,0.8fr)] items-center gap-6">
                ...
                <div className="flex min-w-0 flex-col gap-5">
                  ...
                  <div className="grid grid-cols-2 gap-4 border-y border-border py-4">
                    <AssetMetric label="持仓" color="var(--profit)"><AnimatedMoney value={totalMarketValue} /></AssetMetric>
                    <AssetMetric label="现金" color={cash < 0 ? 'var(--loss)' : undefined}><AnimatedMoney value={cash} /></AssetMetric>
                    <AssetMetric label="当前总资产"><AnimatedMoney value={currentTotalAssets} /></AssetMetric>
                    <AssetMetric label="初始资金"><AnimatedMoney value={totalAssets} /></AssetMetric>
                  </div>
                </div>
              </div>
            )}
            ...
            {overviewPanel === 'sold' && (soldPositions.length ? (
              <Table hoverableRows>
                <TableHeader><TableRow><TableHead>股票</TableHead><TableHead className="text-right">数量</TableHead><TableHead className="text-right">成本 / 卖出</TableHead><TableHead className="text-right">实现盈亏</TableHead><TableHead>卖出时间</TableHead></TableRow></TableHeader>
                <TableBody>{soldPositions.map((item) => <TableRow key={item.id}>
                  ...
                  <TableCell className="text-right tabular-nums"><div>{formatMoney(item.costPrice, item.market)}</div><div className="text-xs text-foreground-muted">{formatMoney(item.sellPrice, item.market)}</div></TableCell>
          ...
          <Dialog open onOpenChange={(open) => { if (!open) setDetailItem(null); }}>
            <DialogContent className="sm:max-w-3xl">
              <DialogHeader>
                ...
                <DialogDescription>{MARKET_LABEL[detailItem.market] || detailItem.market} · 持有 {detailItem.quantity} 股{detailItem.priceUpdatedAt ? ` · 现价更新于 ${new Date(detailItem.priceUpdatedAt).toLocaleString('zh-CN', { hour12: false })}` : ''}</DialogDescription>
              </DialogHeader>
              <DialogBody>
                <div className="grid grid-cols-3 gap-x-6 gap-y-5">
                  <AssetMetric label="浮动盈亏" color={pnl >= 0 ? 'var(--profit)' : 'var(--loss)'} detail={<AnimatedPercent value={pnlPercent} />}><AnimatedMoney value={pnl} market={detailItem.market} /></AssetMetric>
                  <AssetMetric label="市值 / 成本" detail={<AnimatedMoney value={cost} market={detailItem.market} />}><AnimatedMoney value={marketValue} market={detailItem.market} /></AssetMetric>
                  <AssetMetric label="现价 / 成本价" detail={<AnimatedMoney value={detailItem.costPrice} market={detailItem.market} />}><AnimatedMoney value={detailItem.currentPrice} market={detailItem.market} /></AssetMetric>
