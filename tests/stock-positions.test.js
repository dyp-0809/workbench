...
async function createPosition(baseUrl, input = {}) {
  const response = await fetch(baseUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ symbol: 'AAPL', name: '苹果', market: 'US', quantity: 10, costPrice: 150, currentPrice: 180, notes: '', ...input }) });
...
test('持仓字段校验拒绝无效输入', async () => {
  await withHub(async ({ baseUrl }) => {
    ...
    assert.equal((await createPosition(baseUrl, { currentPrice: 'abc' })).response.status, 400);
    const listed = await (await fetch(baseUrl)).json();
